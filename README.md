...
3. 找到 **KV 命名空间绑定 (KV namespace bindings)**，点击 **添加绑定**：
   ...
   - `BOT_KV` 现在只存低频写入的数据（用户偏好、封禁记录、待处理动作等），免费额度完全够用
4. 同一个页面找到 **D1 数据库绑定 (D1 database bindings)**，点击 **添加绑定**：
   - 变量名称填 `DB`
   - 点击 **创建新的数据库 (Create new)**，起个名字比如 `tg-bot-db`，直接创建并绑定
   - D1 免费额度是**每天 10 万次写入**（KV 免费版只有 1,000 次/天），所以聊天记录、限流计数、使用统计这些高频写入的数据都迁移到了这里
5. 同一个页面找到 **Workers AI 绑定**，点击 **添加绑定**：
   - 变量名称填 `AI`
6. 初始化 D1 数据库表结构（只需要做一次）：
   - 回到 Cloudflare 首页，进入 **存储和数据库 (Storage & Databases) → D1**，点击你刚创建的 `tg-bot-db`
   - 点击顶部的 **控制台 (Console)** 标签
   - 打开这个仓库里的 `schema.sql` 文件，把里面的全部内容复制粘贴到控制台的输入框里，点击 **执行 (Execute)**
   - 看到几条 `CREATE TABLE` 语句都执行成功，就说明表已经建好了
7. 保存后，回到 **部署 (Deployments)** 标签页，点击最新一次部署旁边的菜单，选择 **重新部署 (Retry deployment)**，让新绑定生效
...
如果访问报错，回第 4 步检查 `BOT_KV`、`DB` 和 `AI` 绑定是否都已添加，`schema.sql` 是否已执行，以及是否重新部署过。
...
- **性能优化**：把多处可以并行的 KV 读写（用户偏好+对话历史、保存历史+统计计数、群发时的用户列表读取）改成并发执行，减少不必要的等待时间；顺手修掉了三条历史遗留的 TypeScript 类型报错，代码库现在完全干净
...
这个项目同时用到 **KV** 和 **D1** 两种 Cloudflare 存储，分工如下：

| 存储 | 免费额度 | 存的数据 |
|---|---|---|
| **KV**（`BOT_KV`） | 写入 1,000 次/天，读取 10 万次/天 | 用户偏好设置、人设、封禁记录、待处理动作——这些写入频率低，改一次生效很久 |
| **D1**（`DB`） | 写入 10 万次/天，读取 500 万次/天 | 聊天历史、限流计数、每日使用统计、模型调用统计、Neuron 用量、异常检测、Webhook 去重——这些是每条消息都可能触发写入的高频数据 |

如果只用 KV，机器人稍微活跃一点，每天几十条消息就能把 1,000 次写入额度用完；拆分成 KV + D1 之后，两边的免费额度都能覆盖日常使用场景。
