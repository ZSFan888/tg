...
配置好 `ADMIN_USER_IDS` 之后，管理员在 Telegram 里可以用这两个命令：

| 命令 | 功能 |
|---|---|
| `/stats` | 查看累计用户数、今日/近 7 天活跃用户数、今日消息总量 |
| `/broadcast <内容>` | 给所有跟机器人有过互动的用户群发一条系统通知 |

**关于统计范围**：`/stats` 统计的是所有用过 `/start` 或发过消息的用户，不是只统计管理员自己。首次使用建议先跑 `/stats` 确认用户数据已经开始累积。
