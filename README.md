...
## 第 4 步：绑定 D1 和 Workers AI

这个仓库的 `wrangler.jsonc` 只保留了本地开发用的基础配置（没有加 `pages_build_output_dir`），所以生产环境的绑定完全可以在仪表盘上点选完成，不需要改代码文件。现在**所有数据都存在 D1 里**，不再需要 KV：

1. 部署完成后，进入这个 Pages 项目详情页
2. 点击顶部标签栏的 **设置 (Settings) → 函数 (Functions)**
3. 找到 **D1 数据库绑定 (D1 database bindings)**，点击 **添加绑定**：
   - 变量名称填 `DB`
   - 点击 **创建新的数据库 (Create new)**，起个名字比如 `tg-bot-db`，直接创建并绑定
   - 表结构会在部署完成后**自动创建**，不需要手动执行任何 SQL（见下方说明）
4. 同一个页面找到 **Workers AI 绑定**，点击 **添加绑定**：
   - 变量名称填 `AI`
5. 保存后，回到 **部署 (Deployments)** 标签页，点击最新一次部署旁边的菜单，选择 **重新部署 (Retry deployment)**，让新绑定生效
6. 重新部署完成后，打开这个 Pages 项目的访问地址（首页），或者调用一次 `/healthz`，D1 里的表就会自动创建好——不需要额外操作
...
如果访问报错，回第 4 步检查 `DB` 和 `AI` 绑定是否都已添加，以及是否重新部署过。
...
- **修复追问卡在"生成中"**：追问建议的生成任务改用 Cloudflare 的 `waitUntil` 机制注册，避免 Worker 在响应发出后被提前终止导致追问永远替换不上
- **性能优化**：把多处可以并行的数据库读写（用户偏好+对话历史、保存历史+统计计数、群发时的用户列表读取）改成并发执行，减少不必要的等待时间；顺手修掉了三条历史遗留的 TypeScript 类型报错，代码库现在完全干净
- **清理 AI 回答里的 Markdown 符号**：AI 生成的 `**加粗**`、`### 标题`、`` `代码` `` 等符号在 Telegram 里没有被渲染，只会显示成一堆星号和井号，现在统一在展示和保存前清洗成纯文本
- **`/websearch` 改为按钮切换**：不再需要重复发送 `/websearch` 命令，直接点击「已开启」「已关闭」按钮切换，和 `/model` `/settings` 交互方式保持一致
- **状态提示动态化**："正在联网搜索…""思考中…"这类占位提示现在带跳动的省略号动画（每 900ms 循环一次），不再是一句话静止不动，更能体现机器人正在处理
- **语音消息支持**：直接给机器人发语音消息（`voice` 或 `audio`），会先用 Cloudflare Workers AI 的 Whisper Large v3 Turbo 模型转成文字，显示识别结果后再走正常的 AI 回答流程，转换过程中同样有动态省略号提示
...
**关于禁用**：`/ban` 需要提供用户的数字 ID（不是用户名），可以在管理员收到的消息转发信息里查看，或者让用户自己发送 `/start`，机器人数据库里会记录其 ID。被禁用的用户发消息不会有任何 AI 回复，只会收到一条说明禁用状态和解除时间的提示；如果不填分钟数则是永久禁用，直到管理员手动 `/unban`。
...
- 检查 **设置 → 函数 (Functions)**，确认 `DB` 和 `AI` 两个绑定都已经添加
...
用户发 `/setprompt` 后，机器人进入"等待输入"状态（D1 存储，5 分钟过期），下一条消息会被识别为提示词内容并保存，之后对话都会套用这个提示词。
...
现在项目**只用 D1** 存储所有持久化数据，不再需要 KV：

| 数据类型 | 说明 |
|---|---|
| 聊天历史、限流计数、使用统计、Neuron 用量、异常检测、Webhook 去重 | 高频写入，每条消息都可能触发 |
| 用户偏好设置、封禁记录、待处理动作、后台管理员账号与会话、系统设置覆盖项 | 低频写入，用户主动操作时才写一次 |

D1 免费额度是**每天 10 万次写入、500 万次读取**，覆盖日常使用完全没问题。

### 自动建表，无需手动操作

代码里有一个 `ensureSchema()` 函数（`src/db/schema.ts`），里面全部是 `CREATE TABLE IF NOT EXISTS` 语句。每次有人访问首页（触发 `/api/public-status`）、调用 `/healthz`，或者机器人收到一条 Telegram 消息（触发 webhook）时，都会先确保这些表存在，不存在就自动创建。

也就是说：**你只需要在 Cloudflare 后台绑定好 D1 数据库（变量名 `DB`），完全不用手动执行任何 SQL**。仓库里保留的 `schema.sql` 只是给想手动检查表结构、或者需要用命令行 `wrangler d1 execute` 排查问题的人参考，正常使用流程里可以完全忽略它。
